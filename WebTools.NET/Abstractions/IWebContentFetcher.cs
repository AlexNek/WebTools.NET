...
public interface IWebContentFetcher : IAsyncDisposable
{
    ...
    /// </param>
    /// <param name="sanitizeLevel">
    /// Controls which noise tags are removed. Strict (default) removes script, style,
    /// nav, footer, header. Minimal removes only script and style. None skips sanitization.
    /// </param>
    /// <param name="ct">Cancellation token.</param>
    ...
    /// Thrown when <paramref name="maxContentLength"/> is less than or equal to zero.
    /// </exception>
    Task<WebContent> FetchAsAsync(string url, EContentFormat format, int? maxContentLength = null, ESanitizeLevel sanitizeLevel = ESanitizeLevel.Strict, CancellationToken ct = default);
}
